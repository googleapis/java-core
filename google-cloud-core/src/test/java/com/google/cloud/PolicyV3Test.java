...
public class PolicyV3Test {
  ...
  private static final List<String> MEMBERS_LIST_1 = ImmutableList.of(USER, SERVICE_ACCOUNT, ALL_USERS);
  private static final List<String> MEMBERS_LIST_2 = ImmutableList.of(ALL_AUTH_USERS, GROUP, DOMAIN);
  private static final List<Binding> BINDINGS_NO_CONDITIONS =
      ImmutableList.of(
          Binding.newBuilder()
                 .setRole(VIEWER)
                 .setMembers(MEMBERS_LIST_1).build(),
          Binding.newBuilder()
                 .setRole(EDITOR)
                 .setMembers(MEMBERS_LIST_2).build());
  private static final List<Binding> BINDINGS_WITH_CONDITIONS =
          ImmutableList.copyOf(BINDINGS_NO_CONDITIONS).of(
                  Binding.newBuilder()
                          .setRole(VIEWER)
  ...
  private static final Policy FULL_POLICY_V1 =
      Policy.newBuilder()
          .setBindings(BINDINGS_NO_CONDITIONS)
          .setEtag("etag")
          .setVersion(1)
          .build();
  ...
  public void testBuilder() {
    ...
    policy = FULL_POLICY_V3_WITH_VERSION_1.toBuilder().setBindings(BINDINGS_WITH_CONDITIONS).setVersion(3).build();
    assertEquals(BINDINGS_WITH_CONDITIONS, policy.getBindingsList());
    assertEquals("etag", policy.getEtag());
    assertEquals(3, policy.getVersion());
  }
  ...
  public void testIllegalPolicies() {
    ...
    try {
      ...
      assertEquals("getBindings() is only supported with version 1 policies and non-conditional policies", ex.getMessage());
    }
    try {
      FULL_POLICY_V3.toBuilder().addIdentity(Role.editor(), Identity.allUsers());
      fail("getBindings() should cause exception with Policy V3.");
    } catch (IllegalArgumentException ex) {
      assertEquals("addIdentity() is only supported with version 1 policies and non-conditional policies", ex.getMessage());
    }
    try {
      FULL_POLICY_V3.toBuilder().removeIdentity(Role.editor(), Identity.allUsers());
      fail("getBindings() should cause exception with Policy V3.");
    } catch (IllegalArgumentException ex) {
      assertEquals("removeIdentity() is only supported with version 1 policies and non-conditional policies", ex.getMessage());
    }
    try {
      FULL_POLICY_V3.toBuilder().setBindings(FULL_POLICY_V1.getBindings());
      fail("getBindings() should cause exception with Policy V3.");
    } catch (IllegalArgumentException ex) {
      assertEquals("setBindings() is only supported with version 1 policies and non-conditional policies", ex.getMessage());
    }
  }
